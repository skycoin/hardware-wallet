...
int signatures_ok(uint8_t* store_hash)
{
    ...
    sign1[64] = 0;
    if (!recover_pubkey_from_signed_digest((char*)hash, sign1, pubkey1)) {
#if SIGNATURE_DEBUG
    ...
    sign2[64] = 0;
    if (!recover_pubkey_from_signed_digest((char*)hash, sign2, pubkey2)) {
#if SIGNATURE_DEBUG
    ...
    sign3[64] = 0;
    if (!recover_pubkey_from_signed_digest((char*)hash, sign3, pubkey3)) {
#if SIGNATURE_DEBUG
