...
ifeq ($(OS),Windows_NT)
	...
	( cd ./tiny-firmware/vendor && cmd /c 'mklink /d skycoin-crypto ..\..\skycoin-crypto\')
endif
...
tiny-firmware/bootloader/libskycoin-crypto.so:
	...
	cp skycoin-crypto/libskycoin-crypto.so tiny-firmware/bootloader/
	$(MAKE) -C skycoin-crypto clean
...
test: clean firmware tiny-firmware/bootloader/libskycoin-crypto.so ## Run all project test suites.
	export LIBRARY_PATH="$(MKFILE_DIR)/skycoin-crypto/:$(shell echo $$$(LIBRARY_PATH))" && \
	export $(LD_VAR)="$(MKFILE_DIR)/skycoin-crypto/:$(shell echo $$$(LD_VAR))" && \
...
check-coverage: clean ## Generate test coverage reports HTML
	export LIBRARY_PATH="$(MKFILE_DIR)/skycoin-crypto/:$(shell echo $$$(LIBRARY_PATH))" && \
	export $(LD_VAR)="$(MKFILE_DIR)/skycoin-crypto/:$(shell echo $$$(LD_VAR))" && \
	$(MAKE) COVERAGE=1 -C $(MKFILE_DIR)/skycoin-crypto/ coverage && \
