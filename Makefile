...
check-coverage: ## Generate test coverage reports HTML
	make clean
	COVERAGE=1 make -C skycoin-api coverage
	COVERAGE=1 EMULATOR=1 make -C tiny-firmware coverage
	lcov -c --directory . --output-file coverage.info

check-coverage: clean ## Generate test coverage reports console
	export LIBRARY_PATH="$(MKFILE_DIR)/skycoin-api/:$$LIBRARY_PATH"
	export $(LD_VAR)="$(MKFILE_DIR)/skycoin-api/:$$$(LD_VAR)"
	...
	lcov -c --directory . --output-file coverage/coverage.info
