...
clean: ## Delete all files generated by build
	...
	# Clear ./releases
	rm -f releases/skywallet-firmware-v$(VERSION_FIRMWARE).bin
	rm -f releases/skywallet-bootloader-no-memory-protect-v$(VERSION_BOOTLOADER).bin
	rm -f releases/skywallet-bootloader-mem-protect-v$(VERSION_BOOTLOADER).bin
	rm -f releases/skywallet-full-no-mem-protect-$(COMBINED_VERSION).bin
	rm -f releases/skywallet-full-mem-protect-$(COMBINED_VERSION).bin
	rm -f releases/release.md5
