...
clean: ## Delete all files generated by build
	...
	rm -f bootloader-no-memory-protect.bin
	rm -f full-firmware-no-mem-protect.bin
	rm -f full-firmware-memory-protected.bin
	rm -f skycoin-v$(VERSION_FIRMWARE).bin
	rm -f bootloader-no-memory-protect-v$(VERSION_BOOTLOADER).bin
	rm -f bootloader-mem-protect-v$(VERSION_BOOTLOADER).bin
	rm -f combined_bootloader-no-memory-protect-v$(VERSION_BOOTLOADER)-firmware-v$(VERSION_FIRMWARE).bin
	rm -f combined_bootloader-mem-protect-v$(VERSION_BOOTLOADER)_firmware-v$(VERSION_FIRMWARE).bin
	rm -f release.md5
...
release-combined-mem-protect: release-bootloader-mem-protect release-firmware
	...
	cp skycoin-v$(VERSION_FIRMWARE).bin tiny-firmware/bootloader/combine/fw.bin
	cd tiny-firmware/bootloader/combine/ ; $(PYTHON) prepare.py
	mv tiny-firmware/bootloader/combine/combined.bin combined_bootloader-mem-protect-v$(VERSION_BOOTLOADER)_firmware-v$(VERSION_FIRMWARE).bin
